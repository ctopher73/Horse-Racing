  ...
  return {
    ...
    userPreferences:
      context.preferences && context.preferences.aiUsePreferencesInAdvice
        ...
        : null,
    races: context.raceSummaries.map(({ race, summary }) => ({
...
export function insightBestBetsToday(ctx) {
  const bets = ctx.raceSummaries
    .filter(({ summary }) => !summary.chaos.skipRace && summary.betRec.betPass === "Bet" && summary.topPick)
  ...
  if (!bets.length) {
    return {
      ...
      tone: "neutral",
      summary: "No race currently clears a full BET signal — the card is light on standout plays right now.",
      detail: "A full BET call requires the top pick to clear the playable score threshold AND show a positive price edge at current odds. Add more races or refresh odds to see this update.",
      raceNumbers: [],
    };
  }
  ...
  const lines = top.map(
    ({ race, summary }) => `${raceLabel(race)}: ${horseLabel(summary.topPick)} (score ${Math.round(summary.topPick.composite)}, ${summary.confidence.label.toLowerCase()} confidence)`
  );
  ...
  return {
    ...
    title: "Best bets today",
    tone: "positive",
    summary: `${top.length} race${top.length === 1 ? "" : "s"} show a clear BET signal. Top of the list: ${horseLabel(top[0].summary.topPick)} in ${raceLabel(top[0].race)}.${trackNote}`,
    detail: lines.join("\n"),
