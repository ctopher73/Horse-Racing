...
// serializeCoachContext() in src/lib/coach.js) plus the user's question and
...
Each race also includes a "betDecision" field — a stricter, separate filter applied after scoring (label: STRONG BET, BET, LEAN, or PASS, plus reasonsToBet and reasonsForCaution arrays, and an effectiveConfidencePct). This is the newer, more conservative signal and supersedes the older "betPass" field when the two disagree — the app was found to be too willing to suggest bets, so betDecision is intentionally selective. Most races should be LEAN or PASS; treat that as expected and healthy, not as a sign something is broken. When discussing whether to bet a race, lead with betDecision's label and reasons, and mention betPass only if asked about it specifically or to contrast the two.
...
- Never present a recommendation as a guarantee. Bet/pass calls, value flags, confidence levels, and the betDecision label are the model's read, not certainties.
- When a race's betDecision is PASS or LEAN, say so plainly and explain why (using reasonsForCaution) rather than softening it into a maybe-bet — the app is deliberately selective, and that selectivity is the point, not a limitation to talk around.
- Kelly stake numbers are informational, not a confident sizing recommendation, until the app has enough tracked race history to calibrate against (see the bankroll context) — frame them that way if asked about stake sizing.
...
function ruleBasedFallback(context, question) {
  ...
  if (/skip|pass|avoid|chaotic/.test(q)) {
    // betDecision (the stricter, newer filter) is the authoritative signal
    // for what to pass on — falls back to chaos.skipRace only for races
    // that predate betDecision being attached, which shouldn't happen in
    // practice but keeps this defensive.
    const passes = context.races.filter((r) => (r.betDecision ? r.betDecision.label === "PASS" : r.chaos.skipRace));
    if (!passes.length) return "No races are currently flagged to pass on this card.";
    return `Races flagged PASS: ${passes
      .map((r) => `Race ${r.raceNumber} (${(r.betDecision && r.betDecision.reasonsForCaution[0]) || r.chaos.reasons[0] || "flagged"})`)
      .join("; ")}.`;
  }
  ...
  if (/bankroll|stake|unit|money/.test(q)) {
    ...
    return `Suggested stakes across this card total $${total.toFixed(0)}, about ${Math.round((total / bankroll) * 100)}% of your $${bankroll.toFixed(0)} bankroll. Treat these Kelly-derived numbers as informational rather than a confident sizing call until there's enough tracked race history to calibrate against.`;
  }

  // Authoritative "what should I bet" answer now defers to betDecision —
  // only STRONG BET / BET clear the bar, matching the stricter frontend logic.
  const betRaces = context.races.filter((r) => r.betDecision && (r.betDecision.label === "STRONG BET" || r.betDecision.label === "BET"));
  if (!betRaces.length) {
    return "No race currently clears this app's bet bar. That's expected most days — passing marginal races protects your bankroll more than forcing an action.";
  }
  const best = betRaces
    .map((r) => ({ r, h: r.horses.find((h) => h.isTopPick) }))
    .filter((x) => x.h)
    .sort((a, b) => b.h.compositeScore - a.h.compositeScore)[0];
  if (!best) return "No race currently clears this app's bet bar.";
  return `Top bet on the card: #${best.h.programNumber || "?"} ${best.h.name} in Race ${best.r.raceNumber} (${best.r.betDecision.label}, score ${best.h.compositeScore}, ${best.r.confidence.label.toLowerCase()} confidence).`;
}

function describeRace(race) {
  const decisionLabel = race.betDecision ? race.betDecision.label : race.chaos.skipRace ? "PASS" : null;
  if (decisionLabel === "PASS" || race.chaos.skipRace) {
    const reason = (race.betDecision && race.betDecision.reasonsForCaution[0]) || race.chaos.reasons.join(" ");
    return `Race ${race.raceNumber} is a PASS. ${reason}`;
  }
  ...
  const parts = [`Race ${race.raceNumber} — ${decisionLabel || race.betPass}, ${race.confidence.label.toLowerCase()} confidence (${race.confidence.pct}%).`];
